...
* [`src/cljs/tree_sheets/`](src/cljs/tree_sheets/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/tree_sheets/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `tree-sheets` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tree_sheets/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tree-sheets.example
  (:require [tree-sheets.config :as config])
